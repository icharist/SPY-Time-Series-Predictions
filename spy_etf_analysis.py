...
# Change date column to datetime type, dealing with the timezone information in a roundabout way
...
print("*" * 100)
print(
    f"""The date range of SPY is {spy.Date.min()} to {spy.Date.max()} 
       """
)
...
spy['day_return'] = (spy['Close'] - spy['Open']) / spy['Open']
spy['flat_return'] = spy['Close'] - spy['Open']
spy['day_gap'] = (spy['Open'] - spy['Close'].shift(1)) / spy['Close']
spy['flat_gap'] =  spy['Open'] - spy['Close'].shift(1)
...
# %%
seasonal_trend_decomp_plot(
    dataframe=spy, target_series="Close", freq="ME", seasonal_smoother=13, period=12
)
##########################################################################################
# %% [markdown]
# **Conclusion**<br/>
